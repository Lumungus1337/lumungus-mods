...
Der serverseitige Diagnosebefehl `/lumungus migration scan <x> <y> <z>` fuehrt den Nur-Lese-Scan an einem Tom's-Netzblock aus. Er meldet Blockanzahl, konvertierbare Bloecke, nur lesend unterstuetzte Bloecke, Blocker und bei einem vollstaendigen Netz Inventaranzahl, Slots, Gesamtmenge sowie Itemtypen. Die optionale Direktintegration erkennt bei Tom's `2.11.3`, ob ein Kabel-Connector tatsaechlich einen Fernkanal verwendet. Gewoehnliche Kabel-Connectoren duerfen den lokalen Snapshot passieren; aktive Fernkanaele werden ueber alle geladenen, erreichbaren Teilnetze und Dimensionen verfolgt. Wenn Tom's zur Laufzeit installiert ist, nutzt der Snapshot zuerst die von Tom's selbst ermittelte Connector-Inventarliste. Dadurch werden auch Lagerketten aus direkt aneinander angrenzenden Kisten, Faessern und anderen Fabric-Inventaren erfasst, statt nur direkte Nachbarn einzelner Netzbloecke zu zaehlen. Tom's `basic_inventory_hopper` wird fuer den Bestandssnapshot als bekannter Nur-Lese-Netzblock akzeptiert, bleibt aber ohne Konvertierungsziel, bis Lumungus Machines einen eigenen Adapter besitzt. Nicht bereite oder ungeladene Verbindungen sperren die Freigabe weiterhin.
