...
| Server-GameTests | `PASS uat.52` | Alle 45 Storage-Tests bestanden; frei waehlbare Ergebnisanzahl, Rezept-Ausgaberundung und rekursive Mehrfachplaene sind geprueft. |
| Client-GameTest | `PASS uat.52` | Storage-Clientstart, Sprachressourcen und Oberflaechen bestanden. |
...
- [ ] **UAT-16ac Crafting-Plan:** Nach einem JEI-Transfer zeigt eine kompakte Spielnachricht jeden rekursiven Verarbeitungsschritt als `Eingabe -> Ausgabe` mit Gesamtmengen sowie die abschliessend im Raster vorbereitete Endmenge.
